...
Work lands on a feature branch. `no-mistakes` refuses to validate `master`.
...
Windows. Default shell is PowerShell; Git Bash is available. Several installed
skills assume POSIX — notably `wizard`, which emits bash. Adapt or skip rather
than emitting scripts that cannot run here.

The default branch is `master`. Work lands on feature branches.
